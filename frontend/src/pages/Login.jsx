  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', width: '100vw', fontFamily: "'Inter', sans-serif", overflow: 'hidden', transition: 'all 0.8s cubic-bezier(0.4,0,0.2,1)' }}>
      ...
      <div style={{ flex: '0.8', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#FAFAFA', padding: '40px', position: 'relative' }}>
        ...
        <div style={{ width: '100%', maxWidth: '400px', background: theme.panel, padding: '50px 40px', borderRadius: '24px', boxShadow: `0 20px 60px ${theme.shadow}, 0 4px 12px rgba(0,0,0,0.05)`, border: '1px solid rgba(255,255,255,0.8)', position: 'relative', zIndex: 1, animation: isLoaded ? 'fadeIn 1s ease-out 0.3s both' : 'none', transition: 'all 0.8s cubic-bezier(0.4,0,0.2,1)' }}>
          ...
          <h2 style={{ textAlign: 'center', margin: '0 0 8px', fontSize: '28px', fontWeight: 700, color: timeOfDay === 'night' ? '#1565C0' : timeOfDay === 'sunset' ? '#E65100' : timeOfDay === 'noon' ? '#2E7D32' : '#2E7D32', letterSpacing: '-0.5px' }}>Welcome Back</h2>
          <p style={{ textAlign: 'center', margin: '0 0 32px', color: '#666', fontSize: '14px' }}>Sign in to continue to AniWay</p>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: '20px' }}>
              ...
              <input type="email" placeholder="Enter your email" value={email} onChange={e => setEmail(e.target.value)} required style={{ width: '100%', padding: '14px 16px', border: '2px solid #E0E0E0', borderRadius: '12px', fontSize: '14px', outline: 'none', boxSizing: 'border-box', background: '#FFFFFF', transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)', color: '#333' }} onFocus={e => { e.target.style.borderColor = timeOfDay === 'night' ? '#1565C0' : timeOfDay === 'sunset' ? '#E65100' : timeOfDay === 'noon' ? '#66BB6A' : '#43A047'; e.target.style.boxShadow = `0 0 0 4px ${theme.shadow}`; }} onBlur={e => { e.target.style.borderColor = '#E0E0E0'; e.target.style.boxShadow = 'none'; }} />
            </div>
            <div style={{ marginBottom: '28px' }}>
              <label style={{ display: 'block', marginBottom: '8px', color: '#424242', fontSize: '13px', fontWeight: 600 }}>Password</label>
              <input type="password" placeholder="Enter your password" value={password} onChange={e => setPassword(e.target.value)} required style={{ width: '100%', padding: '14px 16px', border: '2px solid #E0E0E0', borderRadius: '12px', fontSize: '14px', outline: 'none', boxSizing: 'border-box', background: '#FFFFFF', transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)', color: '#333' }} onFocus={e => { e.target.style.borderColor = timeOfDay === 'night' ? '#1565C0' : timeOfDay === 'sunset' ? '#E65100' : timeOfDay === 'noon' ? '#66BB6A' : '#43A047'; e.target.style.boxShadow = `0 0 0 4px ${theme.shadow}`; }} onBlur={e => { e.target.style.borderColor = '#E0E0E0'; e.target.style.boxShadow = 'none'; }} />
            </div>
            <button type="submit" style={{ display: 'block', width: '100%', padding: '16px', background: theme.btn, color: '#fff', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: 600, cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.4,0,0.2,1)', marginBottom: '24px', boxShadow: `0 4px 16px ${theme.shadow}`, letterSpacing: '0.5px' }} onMouseOver={e => { e.target.style.background = theme.btnHover; e.target.style.boxShadow = `0 6px 24px ${theme.shadow}`; e.target.style.transform = 'translateY(-2px)'; }} onMouseOut={e => { e.target.style.background = theme.btn; e.target.style.boxShadow = `0 4px 16px ${theme.shadow}`; e.target.style.transform = 'translateY(0)'; }}>Sign In</button>
            <p style={{ textAlign: 'center', color: '#666', fontSize: '14px', margin: 0 }}>Don't have an account?{' '}<span onClick={onGoToSignUp} style={{ color: timeOfDay === 'night' ? '#1565C0' : timeOfDay === 'sunset' ? '#E65100' : timeOfDay === 'noon' ? '#2E7D32' : '#2E7D32', fontWeight: 600, cursor: 'pointer', textDecoration: 'none', borderBottom: '1px solid transparent', transition: 'all 0.3s' }} onMouseOver={e => e.target.style.borderBottomColor = timeOfDay === 'night' ? '#1565C0' : timeOfDay === 'sunset' ? '#E65100' : timeOfDay === 'noon' ? '#2E7D32' : '#2E7D32'} onMouseOut={e => e.target.style.borderBottomColor = 'transparent'}>Sign Up</span></p>
          </form>
        </div>
      </div>
